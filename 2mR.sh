...
# This script is for ## 6m, located in ~/wsprd, and ORIGINALLY NAMED 1mR.sh
...
# Setting band parameter as 1, 2 or 3   
    bandparam=1
    if [ "$bandparam" -eq 1 ]; then    this_parameter="6m" 
    elif [ "$bandparam" -eq 2 ]; then    this_parameter="50.2930M" 
    else    this_parameter="50.293000M" 
    fi
